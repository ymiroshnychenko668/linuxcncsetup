...
func TestValidateRootsRejectsOverlapAndCanonicalizes(t *testing.T) {
	library := t.TempDir()
	...
	if configuration.LibraryDir != library {
		t.Fatalf("canonical library = %q, want %q", configuration.LibraryDir, library)
	}
}
