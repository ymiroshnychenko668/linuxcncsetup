...
(
    cd "$project_dir"
    "$go_binary" test ./...
    CGO_ENABLED=1 "$go_binary" test -tags pam ./...
    CGO_ENABLED=1 "$go_binary" vet -tags pam ./...
    CGO_ENABLED=1 "$go_binary" build -tags 'production pam' -trimpath -ldflags '-s -w' -o "$build_dir/websetupmanager" ./cmd/websetupmanager
)
