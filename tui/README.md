...
terminal workspace header. The source bundle makes this workflow independent
of the original repository checkout; local `node_modules`, build output, and
environment files are never included.
...
The main-menu **Git setup** submenu keeps source-control authentication
separate from the broader developer-tools installer:
