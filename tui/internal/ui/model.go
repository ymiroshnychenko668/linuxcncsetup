...
const (
	...
	actionOpenRemoteTerminal
	actionInstallRemoteTerminal
	actionOpenGitSetup
...
func (m Model) renderDetail() string {
	...
	case actionOpenRemoteTerminal:
		lines = append(lines,
			"Press Enter to name this machine and configure",
...
func (m *Model) prepareSelectedAction() {
	...
	switch current.action {
	case actionOpenRemoteTerminal:
		m.remoteTerminal = newRemoteTerminalDraft()
		m.openPage(menuRemoteTerminal)
		return

	case actionInstallRemoteTerminal:
		if !m.prepareRemoteTerminalInstall() {
			return
		}
...
func (m Model) visibleSections() []section {
	switch m.page {
	case menuRemoteTerminal:
		return remoteTerminalSections
	case menuGitSetup:
...
func (m *Model) back() {
	...
	case menuRemoteTerminal:
		m.openPage(menuMain)
		m.selectAction(actionOpenRemoteTerminal)
	case menuGitSetup:
		m.openPage(menuMain)
...
func (m Model) executeAction(action sectionAction, value string) tea.Cmd {
	...
	case actionInstallRemoteTerminal:
		return m.runRemoteTerminalInstall()
	case actionInstallGitTools:
...
func actionName(action sectionAction) string {
	...
	case actionInstallRemoteTerminal:
		return "Remote Terminal installation"
	case actionInstallGitTools:
...
func actionRunningMessage(action sectionAction) string {
	...
	case actionInstallRemoteTerminal:
		return "Building and installing Remote Terminal with Ansible..."
	case actionInstallGitTools:
...
func actionCancelledMessage(action sectionAction) string {
	...
	case actionInstallRemoteTerminal:
		return "Remote Terminal installation cancelled."
	case actionInstallGitTools:
...
func actionSuccessMessage(action sectionAction) string {
	...
	case actionInstallRemoteTerminal:
		return "Remote Terminal installed successfully."
	case actionInstallGitTools:
