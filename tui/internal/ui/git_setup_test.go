...
func TestGitSetupMainMenuPlacement(t *testing.T) {
	remoteTerminalIndex := mainMenuActionIndex(t, actionOpenRemoteTerminal)
	gitSetupIndex := mainMenuActionIndex(t, actionOpenGitSetup)

	if gitSetupIndex != remoteTerminalIndex+1 {
		t.Fatalf(
			"Git setup is at index %d; want %d immediately after Remote Terminal",
			gitSetupIndex,
			remoteTerminalIndex+1,
		)
	}
