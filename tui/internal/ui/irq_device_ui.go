...
func renderIRQVectorTable(irqs []IRQDeviceIRQ, totals []IRQCPUCount) []string {
	...
	lines := make([]string, 0, len(rows)+len(irqs)+1)
	for rowIndex, row := range rows {
		...
		} else if rowIndex <= len(irqs) {
			description := strings.TrimSpace(irqs[rowIndex-1].Description)
			if description != "" {
...
func (m Model) wrapIRQDeviceLines(lines []string) []string {
	...
	result := make([]string, 0, len(wrappedLines))
	for _, line := range wrappedLines {
		// Keep every physical line within the panel even if Wrap chooses a
		// breakpoint that leaves a one-cell overrun.
		result = append(result, strings.Split(lipgloss.Wrap(line, width, ""), "\n")...)
	}
	return result
}
...
func (m *Model) scrollIRQDeviceDetail(direction int) {
	...
	maximumOffset := 0
	if len(lines) > limit {
		maximumOffset = len(lines) - limit + 1
	}
	step := max(limit-2, 1)
